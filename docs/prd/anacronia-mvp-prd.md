...
14. As a collection builder, I want Collection titles to generate stable slugs, so that folders and identifiers remain clean.
15. As a collection builder, I want no draft Collection saved before I start searching, so that abandoned forms do not clutter the sidebar.
16. As a collection builder, I want `Start search` to create and save the Collection, so that creation and first search are one clear action.
17. As a collection builder, I want the Collection title, terms, and first Provider Source locked after `Start search`, so that the MVP stays reproducible and simple.
18. As a collection builder, I want reusing an existing Collection title to avoid duplicates without silently changing the locked Collection, so that existing work is protected.
19. As a collection builder, I want each input line or comma-separated segment to be treated as one term even if it contains spaces, so that terms like `garden snake` work without quotes.
20. As a collection builder, I want duplicate terms to be deduplicated case-insensitively, so that repeated terms do not create duplicate provider queries.
21. As a collection builder, I want terms to run as separate provider searches, so that Anacronia can record which terms matched which records.
22. As a collection builder, I want provider candidates from multiple terms to be merged and deduplicated before internal cursor and processing limits are applied, so that search continuation stays reproducible.
23. As a collection builder, I want candidate order to follow term insertion order and provider order, so that early terms define priority.
24. As a collection builder, I want a batch dropdown with `5`, `10`, `20`, `30`, `100`, `500`, and `1000`, defaulting to `10`, so that I can start with a small, reviewable amount of usable local material and deliberately choose larger batches when needed.
25. As a collection builder, I want batch size to mean target usable downloaded results, not provider candidates processed, so that the control matches what I receive locally.
26. As a developer, I want internal candidate cursors and limits to remain auditable, so that provider search is reproducible without exposing candidate mechanics in the primary UI.
27. As a collection builder, I want the UI to hide technical Run complexity by default, so that I see one continuing Collection rather than many internal executions.
28. As a collection builder, I want technical details to remain available, so that progress, errors, and resumes can be audited.
29. As a collection builder, I want Anacronia to start searching immediately without manual pre-review, so that large searches do not require me to inspect thousands of records.
30. As a collection builder, I want only public-domain Met records to be accepted in the MVP, so that the first provider has a clear rights rule.
31. As a collection builder, I want future providers to use provider-specific eligibility rules, so that Europeana-style open/reusable records can be handled later.
32. As a collection builder, I want source rights and license statements stored, so that each imported Image Asset retains provenance.
33. As a collection builder, I want the image detail panel to show source rights/license information, so that I can understand the origin of each asset.
34. As a collection builder, I want Anacronia to store raw provider records, so that data can be audited and reprocessed later.
35. As a collection builder, I want Met raw records stored as one JSON file per Museum Object, so that individual records are easy to inspect.
36. As a collection builder, I want a human-readable Met file layout, so that I can browse data folders manually if needed.
37. As a collection builder, I want Met object files grouped into numeric range folders, so that directories remain manageable without becoming opaque.
38. As a collection builder, I want per-object image folders, so that all derivatives for one Museum Object are easy to find.
39. As a collection builder, I want source image filenames and URLs stored in metadata, so that local derivative filenames can stay short and stable.
40. As a collection builder, I want local image filenames standardized, so that the filesystem is predictable.
41. As a collection builder, I want Image Asset identity based on Source Identity rather than provider array position or local row ID, so that additional image order changes, V&A string IDs, local-folder material, delete, and re-import do not create duplicates.
42. As a collection builder, I want duplicate source image URLs within an object deduplicated, so that the same image is not stored twice.
43. As a collection builder, I want Anacronia to import Met objects with valid additional images even when `primaryImage` is missing, so that useful images are not discarded.
44. As a collection builder, I want Met `primaryImageSmall` stored as metadata but not downloaded, so that source information is preserved without redundant local files.
45. As a collection builder, I want `primaryImageSmall` not used as fallback for failed source downloads, so that local image quality stays consistent.
46. As a collection builder, I want valid images on an object attempted up to the MVP per-object image limit, so that additional views are considered without exploding collect size.
47. As a collection builder, I want the MVP per-object image limit to be 3 and not exposed as a routine UI control, so that collection setup stays simple.
48. As a collection builder, I want image URLs beyond the per-object limit stored as skipped references, so that I know more images exist.
49. As a collection builder, I want the detail panel to show when related images were skipped by limit, so that I understand incomplete object image coverage.
50. As a collection builder, I want each Image Asset accepted independently, so that one failed image does not block other images from the same Museum Object.
51. As a collection builder, I want an Image Asset to count as imported only when both derivatives are valid, so that imported counts represent usable local images.
52. As a collection builder, I want `standard-1024` generated for each imported Image Asset, so that I have one consistent local image for analysis and detailed viewing.
53. As a collection builder, I want `thumb-256` generated for each imported Image Asset, so that grids and dense views are fast.
54. As a collection builder, I want source originals deleted after derivative generation, so that disk usage remains controlled.
55. As a collection builder, I want original image width and height captured before deletion, so that technical source metadata is retained.
56. As a collection builder, I want Anacronia to validate local derivatives by readability, dimensions, and processing settings, so that corrupt partial files do not count as imported.
57. As a collection builder, I do not need file hashes in the MVP, so that implementation remains simpler.
58. As a collection builder, I want missing descriptive fields to be allowed, so that records without artist/date/culture are not discarded when images are valid.
59. As a collection builder, I want provider timestamps such as Met `metadataDate` retained, so that I know when provider metadata was current.
60. As a collection builder, I want Anacronia to extract Descriptors from provider-specific fields, so that local search can work across metadata shapes.
61. As a collection builder, I want Descriptors to keep their source field, so that Anacronia can explain where a term came from.
62. As a collection builder, I want Descriptor extraction driven by explicit provider mappings, so that the system is deterministic and not dependent on AI interpretation.
63. As a collection builder, I want raw provider records retained for Descriptor regeneration, so that improved mappings can update existing collections.
64. As a collection builder, I want a command to rebuild Descriptors, so that metadata improvements can be applied after collection.
65. As a collection builder, I want verified matches recorded when terms appear in inspected fields, so that I can understand why a record was collected.
66. As a collection builder, I want unverified matches retained when provider search returns records but the term is not visible in inspected fields, so that provider internal matching is not lost.
67. As a collection builder, I want verified-match logic to use simple case-insensitive substring matching, so that matching is transparent.
68. As a collection builder, I want Met verified matches checked against title, object name, tags, medium, culture, period, classification, and artist display name, so that obvious match reasons are captured.
69. As a collection builder, I want Met Descriptors extracted from a broader curated field set than verified matching, so that later local search is richer.
70. As a future collection browser, I want local post-import search across canonical fields and Descriptors, so that I can find material after collection.
71. As a collection builder, I do not need local result search in the New Collection workflow, so that the first workflow stays focused on choosing a source trajectory and starting work.
72. As a collection builder, I want only one Provider Search actively searching or stopping at a time, so that the MVP remains stable and provider-friendly.
73. As a collection builder, I want no search queue in the MVP, so that system behavior stays simple.
74. As a collection builder, I want stopped and paused/error searches to be parked without blocking other work, so that I can move to another Collection when a search is not actively running.
75. As a collection builder, I want parked searches to resume only when no other Provider Search is searching or stopping, so that resuming remains safe without introducing a queue.
76. As a collection builder, I want `Stop search` to finish the current Museum Object safely before stopping, so that local data is not left corrupt.
77. As a collection builder, I want stopping a search to preserve completed Objects and Images, so that already downloaded material is not lost.
78. As a collection builder, I want resuming or keeping a search going to continue from the next safe internal cursor, so that I do not repeat processed provider records by default.
79. As a collection builder, I want search feedback shown through state plus `Objects` and `Images` counters, so that I understand the usable local material collected.
80. As a collection builder, I want repeated provider failures to trigger slowdown and then pause, so that provider throttling does not cause uncontrolled failure.
81. As a collection builder, I want long-running or overnight collects to be acceptable, so that robustness takes priority over speed.
82. As a collection builder, I want disk space checked before and during Provider Searches, so that the system does not fill my disk.
83. As a collection builder, I want Anacronia to show status and logs in the MVP, so that I can understand completion, pauses, and errors.
84. As a collection builder, I do not need macOS notifications in the MVP, so that the first version stays focused.
85. As a collection builder, I want a dense Collection workspace, so that search state and controls are easy to scan.
86. As a collection builder, I want Collections as the primary navigation, so that the UI follows my research themes.
87. As a collection builder, I want provider sources shown underneath Collections, so that later multi-provider work remains organized.
88. As a collection builder, I want provider-first views available secondarily, so that I can inspect work by museum when needed.
89. As a collection builder, I want a basic object-first grid in the MVP, so that I can quickly see which Museum Objects were downloaded.
90. As a collection builder, I want newest downloaded Objects to appear first, with a carousel indicator for multi-image Objects, so that new results and siblings are easy to spot.
91. As a collection builder, I want object details in a right-side overlay, so that browsing remains fast without navigating away.
92. As a collection builder, I want the overlay to show a `standard-1024` image carousel, essential metadata, source links, match/source info, license info, and skipped-image information, so that each Museum Object is inspectable.
93. As a collection builder, I want a button to open the provider object page, so that I can verify source context.
94. As a collection builder, I want the detail overlay to support close button, outside click, `Esc`, keyboard focus containment, and carousel keyboard navigation, so that object review is usable without breaking browsing focus.
95. As a collection builder, I do not need advanced faceted filtering in the MVP, so that the interface remains focused.
96. As a collection builder, I want exports to include only imported material, so that exported datasets are clean.
97. As a collection builder, I want export rows to be Image Assets, so that multi-image Museum Objects produce one record per usable image.
98. As a collection builder, I want JSONL as the primary export format, so that large datasets are easy to process in Python.
99. As a collection builder, I want CSV as a simplified companion export, so that I can inspect data in spreadsheet tools.
100. As a collection builder, I want optional complete export packages with images plus metadata, so that I can move or share a collected dataset.
101. As a collection builder, I want future full-resolution export workflows possible, so that I can later package provider full-res images for already collected assets.
102. As a future analyst, I want AI/OpenCV/ML outputs stored as separate Analysis Results, so that generated data never overwrites provider metadata.
103. As a future analyst, I want Image Assets to support many Analysis Results, so that multiple analysis pipelines can coexist.
104. As a future analyst, I want Analysis Results to have stable IDs, manifests, recipes, source scope snapshots, artifact lists, sizes, checksums where useful, and provenance, so that latent-map outputs are durable and auditable.
105. As a future analyst, I want Analysis Results to be immutable once produced, so that changing Collection membership or recipe settings creates a new result rather than silently rewriting old work.
106. As a future analyst, I want Analysis Results to reference artifacts through stable logical keys, so that local filesystem storage can later move behind an Artifact Store or cloud object store without changing viewer semantics.
107. As a future hosted-viewer operator, I want an Analysis Result to be exportable as a Project Viewer Export, so that a local analysis can become a private client-facing viewer without exposing local paths or unnecessary originals.
108. As a future user, I want unwanted Museum Objects and Image Assets to be removable from individual Collections or deletable from local Anacronia data later, so that manual curation can remove material I do not want.
109. As an MVP user, I do not want destructive deletion exposed yet, so that the first version avoids dangerous data loss.
110. As a developer, I want a Provider Adapter interface, so that Met, Europeana, V&A, and future sources can share collection workflows.
111. As a developer, I want provider-specific metadata preserved, so that Anacronia does not flatten away useful fields.
112. As a developer, I want a small canonical model, so that the UI and exports work across providers without pretending all museums share one schema.
113. As a developer, I want provider mapping files, so that Descriptor extraction can evolve without rewriting the pipeline.
114. As a developer, I want Next.js as the frontend, so that the UI has a better path toward future online deployment.
115. As a developer, I want shadcn/ui components, so that the interface uses consistent, maintainable UI primitives.
116. As a developer, I want custom UI to be checked against the installed shadcn/ui component library before being built, so that Anacronia does not accumulate unnecessary local component variants.
117. As a project maintainer, I want ChatGPT/Codex-assisted UX and UI work to stay grounded in the PRD, UX contracts, issues, and repo component system, so that AI-assisted iteration does not drift away from the product model.
118. As a project maintainer, I want uncertain UX/UI changes explored through a small set of disposable visual prototypes before production implementation, so that interaction decisions are visible and reviewable before code is committed to the main app.
119. As a developer, I want Next.js to proxy calls to FastAPI where useful, so that the browser UI does not need to know every backend endpoint directly.
120. As a developer, I want FastAPI and Python workers, so that image processing and future OpenCV/ML work stay in the Python ecosystem.
121. As a developer, I want `uv` for Python dependencies and `npm` for the frontend, so that setup remains modern and straightforward.
122. As a developer, I want Homebrew support when available but not an unexplained hard dependency, so that Mac setup can handle different users.
123. As a future project maintainer, I want a terminal-based MVP install with strong documentation, so that the GitHub project is usable before a packaged installer exists.
124. As a future project maintainer, I want installer/desktop packaging left as future work, so that MVP development does not get blocked by packaging.
125. As a collection curator, I want to favorite selected Museum Objects or Image Assets, so that important material is easy to return to.
126. As a collection curator, I want favorites to be global, so that the same Object or Image appears favorited in every Collection and in User Library.
127. As a collection curator, I want to filter favorites in User Library, so that I can quickly review the best material across all local data.
128. As a collection curator, I want to filter favorites inside a Collection, so that I can review favorited material that belongs to the current Collection only.
129. As a collection curator, I want to remove selected Museum Objects from the current Collection, so that the Collection no longer contains unwanted objects.
130. As a collection curator, I want removing a Museum Object from a Collection to remove all of that object's Image Assets from that Collection, so that object-level curation is predictable.
131. As a collection curator, I want to remove selected Image Assets from the current Collection, so that sibling Image Assets can remain when only one image is unwanted.
132. As a collection curator, I want removed material to remain in User Library and other Collections, so that Collection cleanup does not destroy local data.
133. As a collection curator, I want removed material not to be automatically added again by future Provider Searches for that Collection, so that manual cleanup is respected.
134. As a collection curator, I want to delete selected Museum Objects or Image Assets, so that unwanted local material can be removed from User Library, all Collections, the database, and local files.
135. As a collection curator, I want delete to leave exports untouched, so that exported packages remain stable.
136. As a collection curator, I want delete not to create a global never-import-again rule, so that future searches can import the same provider material again if it still matches.
137. As a collection curator, I want orphan Objects and Images with no Collection membership to remain visible in User Library as `No Collection`, so that local material never becomes invisible just because it was removed from Collections.
138. As a collection curator, I want orphan Objects and Images to support the same User Library actions as other local material, so that they can be favorited, exported, selected, or deleted.
139. As a collection curator, I want the selection toolbar action order to be Export, Remove from Collection, Delete, so that selection mode only contains actions that operate on the selected visible material.
140. As a collection curator, I want `Remove from Collection` hidden in User Library, so that actions only appear where they make sense.
141. As a collection curator, I want User Library to provide a `No Collection` filter, so that I can review orphan local material directly.
142. As a collection curator, I want changing local search, Provider filter, Object/Image view, scope, or active Collection to clear selection, so that hidden selected items are not accidentally acted on.
143. As a collection curator, I want `Select all` to select only currently visible loaded results, so that bulk actions do not affect unseen material.
144. As a collection curator, I want Collection detail views and counts to reflect current Collection Membership, so that removed Image Assets do not remain in Collection carousels or counters.
145. As a collection curator, I want User Library detail views and counts to include all active local material, including `No Collection` material, so that User Library stays complete.
146. As a collection curator, I want exports to include favorite state, so that downstream analysis can use my curation marks.
147. As a collection curator, I want delete confirmations to warn when material is shared or favorited, so that destructive scope is clear.
148. As a developer, I want explicit Collection Membership backfilled from current Run/match-derived visibility, so that existing data keeps the same visible results after the curation model is added.
149. As a developer, I want Collection Exclusions and Favorites keyed by Source Identity, so that delete/re-import does not break curation intent across online Provider and local-folder material.
150. As a developer, I want deleted Objects and Images marked inactive/deleted while local files are removed, so that Run history can remain auditable.
151. As a developer, I want re-import after delete to reactivate or update the old inactive Source Identity row, so that duplicate active rows are avoided.
...
- Use the Analysis Studio PRD as the governing contract for durable local Analysis Results, Analysis Scopes, Analysis Recipes, Analysis Jobs, Image Embedding Results, Artifact Store, Latent Space Explorer integration, and Project Viewer Export.
- Treat a durable Analysis Result as the boundary between Library/Collections, Analysis Studio, Latent Space Explorer, and future Project Viewer Export.
- Do not treat prototype latent-map run folders or `/tmp` paths as durable product state once Analysis Results are integrated.
- Keep Analysis Result manifests free of local absolute paths, secrets, temporary staging paths, and machine-specific details that would leak into hosted viewer exports.
- Prepare local artifact keys and manifests for later R2/S3-style object storage without introducing cloud storage in the collection-builder MVP.
...
- **Analysis Result Registry**: Owns durable Analysis Result records, immutable manifests, scope snapshots, recipe provenance, artifact accounting, and deletion/retention decisions for analysis outputs.
- **Artifact Store**: Owns stable logical keys for analysis artifacts and hides whether artifacts are stored on the local filesystem or, later, in object storage.
- **Analysis Studio**: Owns scope selection, recipe selection, job progress, stale-result notices, and browsing/opening existing Analysis Results. The detailed requirements live in the Analysis Studio PRD.
- **Latent Space Explorer**: Owns visual exploration of existing Analysis Results and should read viewer data from the Analysis Result artifact contract rather than from ad hoc prototype run paths.
...
- Analysis Result Registry tests should prove immutable result creation, manifest completeness, scope snapshot recording, recipe/provenance recording, artifact size/checksum accounting, and delete behavior that leaves source Image Assets and reusable Image Embedding Results intact by default.
- Artifact Store tests should prove artifacts can be written, read, listed, deleted by stable key, and exported without leaking local absolute paths.
- Latent Space Explorer data-contract tests should prove the viewer opens Analysis Results through manifests and artifact keys instead of relying on disposable prototype run folders.
- Project Viewer Export tests should prove the export package includes required viewer artifacts, excludes originals and local paths by default, and validates independently from the local Anacronia app.
...
- OpenCV, embeddings, clustering, visual similarity, segmentation, generated semantic metadata, or other Analysis Result pipelines remain outside the original collection-builder MVP, but the Analysis Studio PRD now governs the next integrated local analysis phase.
- The immersive visual atlas, WebGL maps, spatial clustering, or advanced image exploration remain outside the original collection-builder MVP, but the Latent Space Explorer is now defined as the viewer for durable Analysis Results in the Analysis Studio PRD.
...
- Online/cloud deployment. ADR-0026 only requires cloud-compatible local boundaries and Project Viewer Export; it does not add hosted SaaS, client upload, or remote processing to this MVP.
