...
def test_query_filters_inactive_neighbors_and_fetches_deep_enough(tmp_path):
    run = create_vector_run(tmp_path)
    rows = [
        json.loads(line)
        for line in (run.run_dir / "manifest.jsonl").read_text().splitlines()
    ]
    rows.append(
        {
            "image_id": "img-d",
            "source_path": str(run.source_folder / "d.jpg"),
            "relative_path": "d.jpg",
            "thumbnail_path": "thumbnails/img-d.jpg",
        }
    )
    (run.run_dir / "manifest.jsonl").write_text(
        "".join(json.dumps(row) + "\n" for row in rows),
    ...
    neighbors = query_faiss_neighbors(
        ...
        image_id="img-a",
        top_k=2,
        active_image_ids={"img-a", "img-c", "img-d"},
    )

    assert [neighbor.image_id for neighbor in neighbors] == ["img-c", "img-d"]


def test_query_relations_rejects_inactive_anchor(tmp_path):
    run = create_vector_run(tmp_path)
    build_faiss_index(
        run_dir=run.run_dir,
        recipe_name="dinov3_vits_256",
