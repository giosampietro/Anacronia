...
Durable output generated by Anacronia through image analysis, AI, machine learning, OpenCV, embeddings, clustering, layouts, or similar processes. Analysis Results are separate from provider metadata and must not overwrite or blur the provenance of provider records.

An Analysis Result belongs to an Analysis Scope snapshot and one Analysis Recipe. It must have a stable ID, creation status, recipe/provenance metadata, input and output counts, artifact manifest, artifact sizes, and enough version information to decide whether it can be trusted, opened, exported, deleted, or superseded by a new result.

Latent-map Analysis Results are the durable boundary between Library/Collections, Analysis Studio, and the Latent Space Explorer. Collections provide source Image Assets; Analysis Studio creates Analysis Results; the Latent Space Explorer opens existing Analysis Results. The Explorer must not depend on disposable prototype run folders as the source of truth.

Analysis Results are immutable once produced. If a Collection changes, new images require a new Analysis Result to be included in embeddings, FAISS, UMAP, clustering, atlases, and viewer layout. Removed or deleted images should be hidden gracefully from old results while preserving historical audit metadata where useful.

### Analysis Scope

The population definition for an Analysis Result. The first integrated scopes are one Collection and multiple Collections. Future scopes may include the whole User Library. An Analysis Scope is resolved into a snapshot of active Image Assets, deduplicated by Source Identity or Image Asset identity, with contributing Collections recorded for interpretation.
...
A user-triggered background execution over one Analysis Scope and one or more Analysis Recipes. One Analysis Job can produce multiple sibling Analysis Results. Jobs track stages, progress, failures, elapsed time, and approximate artifact size without turning the Latent Space Explorer into the computation owner.

### Image Embedding Result

A reusable per-Image Asset embedding generated for a specific model, preprocessing configuration, and derivative input. Scope-level Analysis Results may reuse Image Embedding Results instead of recomputing expensive embeddings when possible. Image Embedding Results are not duplicated into each Analysis Result unless a future export format explicitly packages them.

### Analysis Artifact

A file or object produced or consumed by an Analysis Result, such as embeddings, FAISS indexes, UMAP layouts, HDBSCAN labels, method comparison reports, viewer manifests, atlas pages, thumbnails, previews, diagnostics, or export packages. Each artifact should have a logical role, stable key, content type, size, optional checksum, and retention class.

### Artifact Store

The boundary that stores and retrieves Analysis Artifacts by stable logical keys. The first implementation can be the local filesystem, but keys and metadata should be cloud-compatible so a later R2/S3-backed store does not require rewriting analysis algorithms or viewer contracts. Analysis and viewer code should not rely on arbitrary local absolute paths.

### Project Viewer Export

A portable package generated from a durable Analysis Result for use outside the local Anacronia app, including only the artifacts required by the viewer. A Project Viewer Export should include manifest, layout, cluster data, relation data needed by the UI, atlas/thumb/preview assets, minimal item metadata, method provenance, and validation output. It must exclude secrets, unnecessary raw provider records, machine-local paths, and originals unless explicitly requested.
